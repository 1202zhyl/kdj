...
//                    alias.put(tableAlias, "");// just store alias
    ...
    private void prepareToParseCurrentNodeAndChilds(ASTNode ast){
        System.out.println("prepareToParseCurrentNodeAndChilds: " + ast.getToken());
...
//                    nowQueryTable = nowQueryTable
    ...
    public static void main(String[] args) throws IOException, ParseException, SemanticException {
        ...
        String sql24 = "SELECT adr.storename, sum(oo1.sale_money_sum) AS s " +
                ...
                "WHERE oo1.order_type_code = '25'  GROUP BY adr.storename   ORDER BY s DESC";
        String sql25 = "DROP TABLE recommendation.user_preference_category_visit";
        String sql26 = "CREATE TABLE recommendation.user_preference_category_visit as  " +
                " SELECT category_sid,date,max(count) pv FROM " +
                " ( select count(*) count,substr(event_date,0,10) date,category_sid  from recommendation.user_behavior_raw_data  " +
                " WHERE member_id IS NOT NULL AND member_id <> 'null' and   " +
                "dt >'$datebeg'  group by category_sid,substr(event_date,0,10),member_id having count<50) ucdata GROUP BY date,category_sid";

        String sql27 = "INSERT overwrite TABLE recommendation.user_behavior_raw_data_cm PARTITION (dt='$yestoday')  ";
        ...
        String sql29 = "INSERT INTO TABLE recommendation.user_behavior_raw_data partition(dt='$yestoday')\n" +
                ...
                "   WHERE dt ='$yestoday' ) apse\n" +
                "INNER JOIN sourcedata.s06_pcm_mdm_goods goods ON goods.sid = apse.product_id\n" +
                "INNER JOIN\n" +
                "  (SELECT DISTINCT product_id,\n" +
                "                   category_id\n" +
                "   FROM idmdata.dim_management_category) dpc ON goods.pro_sid = dpc.product_id\n" +
                "LEFT JOIN\n" +
        ...
        String sql30 = "create table recommendation.goods_basic_info_temp as\n" +
